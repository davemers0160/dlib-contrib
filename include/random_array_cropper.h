#ifndef DLIB_RaNDOM_ARRAY_CROPPER_H_
#define DLIB_RaNDOM_ARRAY_CROPPER_H_
...
namespace dlib
{
    ...
    public:
        ...
        const dlib::chip_dims& get_chip_dims(
        ) const { return dims; }

        void set_chip_dims (
            const dlib::chip_dims& dims_
        ) { dims = dims_; }
        ...
        // this does nothing, but allows the same code for both this cropper and the enhanced cropper
        ...
        )
        {
            ...
            make_crop_plan(img[0], rects, crop_plan, should_flip_crop);

            for(int idx=0; idx<img.size(); ++idx)
                extract_image_chip(img[idx], crop_plan, crop[idx]);
            ...
            for (auto rect : rects)
            {
                ...
                if (get_rect(crop[0]).intersect(rect.rect).area() != 0)
                {
                    // set to ignore if not totally in the crop or if too small.
                    if (!get_rect(crop[0]).contains(rect.rect) ||
            ...
            {
                image_type2 temp;
                for (uint32_t idx = 0; idx < crop.size(); ++idx)
                {
                    flip_image_left_right(crop[idx], temp[idx]);
                    swap(crop[idx], temp[idx]);
                }
                for (auto&& rect : crop_rects)
                    rect.rect = impl::flip_rect_left_right(rect.rect, get_rect(crop[0]));
            }
    ...
    private:
        ...
        )
        {
            ...
            crop_plan = dlib::chip_details(crop_rect, dims, angle);
        }
...
// ----------------------------------------------------------------------------------------
    ...
    inline std::ostream& operator<< (
        std::ostream& out,
        const random_array_cropper& item
    )
    {
        using std::endl;
        out << "random_array_cropper details: " << endl;
...
#endif // DLIB_RaNDOM_ARRAY_CROPPER_H_
